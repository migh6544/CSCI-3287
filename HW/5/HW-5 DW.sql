...
SELECT * FROM fact_productsales F;
SELECT * FROM dim_date D; 	-- D.DateKey = F.SalesDateKey 
SELECT * FROM dim_product P;	-- P.ProductKey = F.ProductID
SELECT * FROM dim_salesperson SP; -- SP.salespersonid = F.salespersonid
SELECT * FROM dim_store S;	-- S.storeid = F.storeid
SELECT * FROM dim_customer C;	-- C.customerid = F.customerid


-- 1
SELECT AnswerOne.CustomerName AS "Customer Name",
AnswerOne.Gender AS "Customer Gender",
AnswerOne.SalesPersonName AS "Sales Person Name",
AnswerOne.City AS "Sales Person City"
FROM (SELECT ps.ProductID,
ps.SalesPrice,
ps.Quantity,
c.CustomerName,
c.Gender,
...
sp.City,
d.DATE,
SUM(ps.SalesPrice) AS TotalSalesPrice,
SUM(ps.Quantity) AS TotalQuantity
FROM Fact_ProductSales ps
LEFT JOIN Dim_Date d ON d.DateKey = ps.SalesDateKey
LEFT JOIN Dim_Product p ON p.ProductKey = ps.ProductID
LEFT JOIN Dim_SalesPerson sp ON sp.SalesPersonID = ps.SalesPersonID
LEFT JOIN Dim_Store s ON s.StoreID = ps.StoreID
LEFT JOIN Dim_Customer c ON c.customerid = ps.customerid
GROUP BY ps.ProductID, ps.SalesPrice, ps.Quantity, c.CustomerName, c.Gender, sp.SalesPersonName, sp.City, d.DATE
	HAVING YEAR(d.DATE) = 2015
		AND SUM(ps.SalesPrice) > 20 AND SUM(ps.Quantity) > 8) AS AnswerOne
ORDER BY TotalQuantity DESC;

-- 2
SELECT AnswerTwo.StoreName AS "Store Name",
AnswerTwo.City AS "Store City",
AnswerTwo.ProductName AS "Product Name"
FROM (SELECT s.StoreName,
s.City,
p.ProductName,
ps.ProductCost,
d.DATE
FROM Fact_ProductSales ps
LEFT JOIN Dim_Store s ON ps.StoreID = s.StoreID
LEFT JOIN Dim_Product p ON p.ProductKey = ps.ProductID
LEFT JOIN Dim_Date d ON ps.SalesDateKey = d.DateKey
GROUP BY s.StoreName, s.City, p.ProductName, ps.ProductCost, d.DATE
	HAVING YEAR(d.DATE) = 2017 AND MONTH(d.Date) = 3
		AND ps.ProductCost < 50 AND s.City = 'Boulder') AS AnswerTwo
ORDER BY AnswerTwo.City;

-- 3
SELECT sp.SalesPersonName AS "Sales Person Name", SUM(ps.SalesPrice * ps.Quantity) AS "Total Revenue"
FROM Fact_ProductSales ps
LEFT JOIN Dim_SalesPerson sp ON ps.SalesPersonID = sp.SalesPersonID
LEFT JOIN Dim_Date d ON ps.SalesDateKey = d.DateKey
WHERE YEAR(d.DATE) = 2017
GROUP BY sp.SalesPersonName
ORDER BY SUM(ps.SalesPrice * ps.Quantity) DESC
LIMIT 2;

-- 4
SELECT c.CustomerName, SUM(ps.SalesPrice * ps.Quantity) AS "Total Revenue"
FROM Fact_ProductSales ps
LEFT JOIN Dim_Customer c ON ps.CustomerID = c.CustomerID
LEFT JOIN Dim_Date d ON ps.SalesDateKey = d.DateKey
WHERE YEAR(d.DATE) = 2017
GROUP BY c.CustomerName
ORDER BY SUM(ps.SalesPrice * ps.Quantity)
LIMIT 1;

-- 5
SELECT s.StoreName, SUM(ps.SalesPrice) AS "Total Sales Price"
FROM Fact_ProductSales ps
LEFT JOIN Dim_Store s ON ps.StoreID = s.StoreID
LEFT JOIN Dim_Date d ON ps.SalesDateKey = d.DateKey
WHERE YEAR(d.DATE) BETWEEN 2010 AND 2017
GROUP BY s.StoreName
ORDER BY s.StoreName ASC;

-- 6
SELECT s.StoreName, p.ProductName, (SUM(ps.SalesPrice * ps.Quantity) - SUM(ps.ProductCost)) AS "Total Profit"
FROM Fact_ProductSales ps
LEFT JOIN Dim_Store s ON ps.StoreID = s.StoreID
LEFT JOIN Dim_Product p ON ps.ProductID = p.ProductKey
LEFT JOIN Dim_Date d ON ps.SalesDateKey = d.DateKey
WHERE p.ProductName LIKE '%Jasmine Rice%' AND YEAR(d.DATE) = 2010
GROUP BY s.StoreName, p.ProductName;

-- 7
SELECT s.StoreName, QUARTER(d.DATE) AS QuarterNo, SUM(ps.SalesPrice * ps.Quantity) AS TotalRevenue
FROM Fact_ProductSales ps
LEFT JOIN Dim_Store s ON ps.StoreID = s.StoreID
LEFT JOIN Dim_Date d ON ps.SalesDateKey = d.DateKey
WHERE s.StoreName = 'ValueMart Boulder' AND YEAR(d.DATE) = 2016
GROUP BY S.StoreName, QuarterNo
ORDER BY QuarterNo;

-- 8
SELECT c.CustomerName, SUM(ps.SalesPrice) AS "Total Sales Price"
FROM Fact_ProductSales ps
LEFT JOIN Dim_Customer c ON ps.CustomerID = c.CustomerID
WHERE c.CustomerName IN ('Melinda Gates', 'Harrison Ford')
GROUP BY c.CustomerName;

-- 9
SELECT s.StoreName, ps.SalesPrice, ps.Quantity
FROM Fact_ProductSales ps
LEFT JOIN Dim_Store s ON ps.StoreID = s.StoreID
LEFT JOIN Dim_Date d ON ps.SalesDateKey = d.DateKey
...
-- 11
SELECT p.ProductName, (SUM(ps.SalesPrice * ps.Quantity) - SUM(ps.ProductCost)) AS "Total Profit"
FROM Fact_ProductSales ps
LEFT JOIN Dim_Product p ON ps.ProductID = p.ProductKey
GROUP BY p.ProductName
ORDER BY (SUM(ps.SalesPrice * ps.Quantity) - SUM(ps.ProductCost)) DESC
LIMIT 3;

-- 12
SELECT YEAR(d.Date) AS Years, MONTHNAME(d.Date) AS MonthsName, SUM(ps.Quantity * ps.SalesPrice) AS TotalRevenue
FROM Fact_ProductSales ps
INNER JOIN Dim_Date d ON ps.SalesDateKey = d.DateKey
WHERE YEAR(d.Date) = 2017 AND MONTH(d.Date) IN (1, 2, 3)
GROUP BY Years, MonthsName
ORDER BY TotalRevenue;

-- 13
SELECT p.ProductName, ROUND(AVG(ps.ProductCost), 2) AS AvgProductCost, ROUND(AVG(ps.SalesPrice), 2) AS AvgSalesPrice
FROM Fact_ProductSales ps
INNER JOIN Dim_Product p ON ps.ProductID = p.ProductKey
INNER JOIN Dim_Date d ON ps.SalesDateKey = d.DateKey
WHERE YEAR(d.Date) = 2017
GROUP BY p.ProductName;
...
-- 14
SELECT c.CustomerName, ROUND(AVG(ps.SalesPrice), 2) AS AvgSalesPrice, ROUND(AVG(ps.Quantity), 2) AS AvgQuantity
FROM Fact_ProductSales ps
INNER JOIN Dim_Customer c ON ps.CustomerID = c.CustomerID
WHERE c.CustomerName = 'Melinda Gates'
GROUP BY c.CustomerName;


-- 15
SELECT s.StoreName, ROUND(MAX(ps.SalesPrice), 2) AS MaxSalesPrice, ROUND(MIN(ps.SalesPrice), 2) AS MinSalesPrice
FROM Fact_ProductSales ps
INNER JOIN Dim_Store s ON ps.StoreID = s.StoreID
